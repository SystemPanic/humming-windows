...
def test_f16_accum(
    ...
    if a_dtype == dtypes.float16:
        if b_dtype.is_floating_point_type and b_dtype.exponent_bits == 5:
            return
    ...
    weight_scale_orig = weight_scale
    ...
    if weight_scale_group_size > 0:
        outputs_ref = inputs_ref.half().matmul(weight_ref.half().T).half()
    else:
        weight_ref = weight_ref / (weight_scale_orig.float() + 1e-30)
